...
Windows で `LINE_MCP_EXTENSIONS=1` を設定すると、**29 個のツール**を利用できます。設定しない場合は **5 個の標準ツール**が記述子として残り、名前、順序、入力スキーマは変わりません。macOS も同じ 5 個を表示しますが、このリリースでは読み取り、送信、ファイル操作を利用できません。
